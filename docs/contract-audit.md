...
| Companion browser extension | ABSENT→WIRED-UNVERIFIED (partial) | `index.html`: "Browser extension" command-palette entry + a new dialog (`dialogExtensionInstall`) added by `scripts/wire-extension-install.mjs` | Real, independent extension package at `extension/` (manifest, popup, options, install pages) plus `app/src/main/protocol.ts` (single-instance lock, `ytdlp-studio://` argv parsing/validation, a self-registered preload exposing `window.ytdlpStudioExtension`, and the two `extension-install:*` IPC handlers) bundled via `electron-builder.yml` `extraResources` | The **extension itself** was loaded via a real "Load unpacked" flow (native folder picker, not a `--load-extension` flag) in an actual Chrome instance and confirmed to register with no errors; its popup/options/install pages were navigated directly and confirmed to render and behave correctly (honest empty-state copy for a non-http(s) page, and the "Confirm before sending" option surviving a real reload via `chrome.storage.local`). The **desktop app's receiving half** (protocol.ts, the guided-install dialog, the incoming-URL-to-Easy-mode wiring) is typechecked and the renderer generator was proven to run cleanly with it registered, but was not exercised in a running packaged/dev build — that would need building and launching the app, which was out of this pass's scope. | First real implementation of this feature in this repository. Not marked SHIPPED because the app side of the round trip (receiving a link and pre-filling Easy mode) has not actually been watched happen end to end; re-verify once the app can be launched with the extension installed and a live click tested. |
